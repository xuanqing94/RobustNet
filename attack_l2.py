...
def attack(input_v, label_v, net, c, TARGETED=False):
    ...
    adverse = input_v.data.clone() #torch.FloatTensor(input_v.size()).zero_().cuda()
    adverse_v = Variable(adverse, requires_grad=True)
    optimizer = optim.Adam([adverse_v], lr=0.01)
    for _ in range(300):
        optimizer.zero_grad()
        ...
        #print(error.size())
        if TARGETED:
            error += c * torch.sum(other - real)
        else:
            error += c * torch.sum(real - other)
        error.backward()
        optimizer.step()
    return adverse_v
